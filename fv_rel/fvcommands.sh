#!/bin/bash
echo $1 $2
FV_DIR=~/Dropbox/sim_rel/flowvisor/sbin

#no matter what value to set for cont_ip sws are conned sucessfully
CONT_INTF_IP='192.168.56.1' #'10.0.0.7'

if [ $1 = 'run' ]
then
        sudo -u mehmet $FV_DIR/./flowvisor
elif [ $1 = 'run-d' ]
then
        sudo -u mehmet $FV_DIR/./flowvisor -d DEBUG
elif [ $1 = 'run-l' ]
then
        sudo -u mehmet $FV_DIR/./flowvisor -l
elif [ $1 = 'ls' ]
then
...
#Note: dpid s are assigned by FV according to the numbers assigned in Mininet to the sws
        fvctl -f fvpasswd_file list-datapath-info 00:00:00:00:00:00:00:0c #dpid s -> 8-byte
...
then
        ...
        fvctl -f fvpasswd_file add-flowspace acterflowspace all 1 dl_type=0x800 acterslice=7 #,myslice2=0, tp_dst=6000
        fvctl -f fvpasswd_file add-flowspace scherflowspace all 20 dl_type=0x800,nw_proto=17,tp_dst=7000 scherslice=7 #,myslice2=0
