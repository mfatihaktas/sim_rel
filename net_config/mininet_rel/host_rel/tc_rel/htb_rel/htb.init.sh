...
#	BURST=15k	 BURST=15k	 CEIL=5Mbit	    CEIL=5Mbit      CEIL=5Mbit
#		                     BURST=15k	    BURST=15k
...
HTB_PATH=${HTB_PATH:-$4/$3}
HTB_CACHE=${HTB_CACHE:-$4/$3/htb.init}

### Uncomment for sed/find with less features (useful for busybox)
#HTB_BASIC="yes"

### Uncomment to enable logfile for debugging
HTB_DEBUG="$4/$3/htblog-$3"
...
if [ -z "$HTB_BASIC" ]; then
	...
	htb_class_list () {
		...
		for dev in `htb_device_list`; do
		  #find $HTB_PATH -type f -name "$dev-*" | sed -n "p"
...
if [ $5 = "add_root" ]; then
  ...
  for dev in $DEVICES; do
	  ...
	  tc qdisc add dev $dev root handle $ROOTID htb \
...
echo "htb_class_list=" `htb_class_list`
for classfile in `htb_class_list`; do
  echo "for classfile=" $classfile
	htb_load_class $classfile
	echo "DEVICE=" $DEVICE
  echo "CLASS=" $CLASS
  echo "PARENT=" $PARENT
	...
	if [ "$LEAF" != "none" ]; then
		...
		parent $ROOTID:$CLASS handle $CLASS $LEAF $LEAFPARM ||
	...
	for mark in `htb_cfile_rules MARK`; do
		...
		tc filter add dev $DEVICE parent $ROOTID:0 protocol ip \
		prio $PRIO_MARK handle $mark fw classid $ROOTID:$CLASS
	done ### mark
	...
	for realm in `htb_cfile_rules REALM`; do
		...
		${DREALM:+to $DREALM} classid $ROOTID:$CLASS
	done ### realm
	...
	for rule in `htb_cfile_rules RULE`; do
		...
		tc filter add dev $DEVICE parent $ROOTID:0 protocol ip \
		prio $PRIO_RULE u32 $u32_s $u32_d classid $ROOTID:$CLASS
	done ### rule
...
*)
	echo "Usage: `basename $0` {start|compile|stop|restart|timecheck|list|stats|minstop}"
esac
