...
class ThreadedTCPRequestHandler(SocketServer.BaseRequestHandler):
  def handle(self):
    ...
    s._call_back(msg_) #msg_=[type_,data_]
################################################################################
sctag_smsgtypes_dict = {'scher-acter':{'send_type':['sp_sching_dec'],
                                       'recv_type':['sp_sching_reply'] },
                        'acter-scher':{'send_type':['sp_sching_reply'],
                                       'recv_type':['sp_sching_dec'] },
                       }
...
def check_smsg(acttype, sctag, msg):
    ...
    #
    Comm protocol scher->acter is as follows;
    msg = {'type':<>, 'data':<> }
    type:
      scher          <->  acter:
      sp_sching_dec       sp_sching_reply
      ...
    data: random, no defed protocol. Will be handled by reged callback method.
    '''
    try:
